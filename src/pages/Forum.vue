<template>
    <div>
        ...
        <div class="container pt-5" v-if="loggedIn() && user().role === 'owners'">
            <div class="row">
                <div class="col-lg-12">
                        ...
                        <div class="ibox-content forum-container">
                            ...
                            <div class="forum-title">
                                <div class="pull-right forum-desc text-right" v-if="mode === 0">
                                    <small>{{ $t('pages.forum.all_posts') }}: 320,800</small>
                                </div>
                                <h3>{{ selectedDiscussion ? selectedDiscussion.name : $t('pages.forum.forums') }}</h3>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-md-9">
                                        <div class="forum-icon">
                                            <font-awesome-icon :icon="discussion.icon" size="3x" />
                                        </div>
                            ...
                            <div v-if="mode === 2">
                                ...
                                <div class="inner-main-body p-2 p-sm-3 forum-content">
                                    <div class="card mb-2" v-for="(post, index) in selectedTopic" v-bind:key="index">
                                        <div class="card-body">
                                            <div class="media forum-item">
                                                <a href="javascript:void(0)" class="card-link">
                                                    <img :src="post.user.picture" class="rounded-circle" style="width: 100%;" width="50" alt="User" />
                                                    <small class="d-block text-center text-muted"><b><span :class="['role-' + post.user.role]">{{ $t('roles.' + post.user.role) }}</span></b></small>
                                                </a>
                                                <div class="media-body ml-3">
                                                    <a href="javascript:void(0)" class="text-secondary">{{ post.user.username }}</a>
                                                    <small class="text-muted ml-2">1 hour ago</small>
                                                    <div class="mt-3 font-size-sm" v-html="post.message">
    ...
    created() {
        if(this.$route.params.discussion && !this.$route.params.topic) {
            ...
            if(this.discussions && this.discussions.length === 0) {
                ...
                this.$store.dispatch('forum/getDiscussions').then(() => {
